...
static const char *
    telnet_init(void *, void **, Config *, char *, int, char **, int, int);
static void telnet_free(void *);
static void telnet_reconfig(void *, Config *);
static int telnet_send(void *, char *, int);
#define telnet_sendbuffer       raw_sendbuffer
static void telnet_size(void *, int, int);
static void telnet_special(void *, Telnet_Special);
static const struct telnet_special *telnet_get_specials(void *);
#define telnet_connected        raw_connected_checks
/* Telnet doesn't transmit exit codes back to the client */
#define telnet_exitcode         raw_exitcode
#define telnet_sendok           raw_sendok
static int telnet_ldisc(void *, int);
static void telnet_provide_ldisc(void *, void *);
#define telnet_provide_logctx   raw_provide_logctx
#if (TELNET_MAX_BACKLOG==RAW_MAX_BACKLOG)
#define telnet_unthrottle raw_unthrottle
#else
...
#endif
#define telnet_cfg_info         raw_cfg_info

/* member functions in plug function table */
#define telnet_log              raw_log
#define telnet_closing          raw_closing
static int telnet_receive(Plug, int, char *, int);
#define telnet_sent             raw_sent

#if (TELNET_MAX_BACKLOG==RAW_MAX_BACKLOG)
#define c_write_telnet(telnet, buf, len) c_write((Raw)telnet, buf, len)
#else
static void c_write_telnet(Telnet telnet, char *buf, int len)
{
    int backlog;
...
#define ADDTOBUF(c) do { \
    ...
    if (outbuflen)
        c_write_telnet(telnet, outbuf, outbuflen);
    sfree(outbuf);
}
